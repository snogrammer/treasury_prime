...
module OpenapiClient
  class ApplyPersonApplicationGet200ResponseInner
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ApplyPersonApplicationGet200ResponseInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ApplyPersonApplicationGet200ResponseInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [first_name, secondary_email_address, bankdata, phone_number, mailing_address, occupation, physical_address, person_id, middle_name, gov_id, document_ids, id, citizenship, date_of_birth, last_name, user_id, email_address, userdata].hash
    end
